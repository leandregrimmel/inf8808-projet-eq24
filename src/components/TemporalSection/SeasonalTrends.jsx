...
const SeasonalTrends = ({ data, metric = "spotifyPopularity" }) => {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr(
        ...
        (d) =>
          r(d.value) *
          Math.cos(angle(d.month) + angle.bandwidth() / 2 - Math.PI / 2)
      )
      .attr(
        "cy",
        (d) =>
          r(d.value) *
          Math.sin(angle(d.month) + angle.bandwidth() / 2 - Math.PI / 2)
      )
      .attr("r", 4)
      .attr("fill", "orange")
      .on("mouseover", (event, d) => {
        d3.select("#tooltip-radial")
          .style("opacity", 1)
          .html(
            `<strong>${d.month}</strong><br/>Avg ${metric}: ${d.value.toFixed(
              1
            )}`
          )
  ...
  return (
    <div style={{ position: "relative" }}>
      <p>
        Ce graphique radial permet d'identifier les tendances saisonnières en
        fonction du mois ou de la saison, en cliquant sur une section pour
