...
const AnnualTrends = ({ data, initialYear }) => {
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    if (!elementsRef.current.path) {
      ...
      elementsRef.current.rScale = d3.scaleLinear().range([innerRadius, outerRadius]);
      elementsRef.current.angleScale = d3.scaleBand()
      ...
      elementsRef.current.labels = g.selectAll("text.label")
        .data(monthlyData)
        .enter()
        .append("text")
        .attr("class", "label")
      ...
      elementsRef.current.path = g.append("path").attr("fill", "none").attr("stroke", "#3b82f6").attr("stroke-width", 3);
      elementsRef.current.points = g.selectAll("circle.point")
        .data(monthlyData)
        .enter()
        .append("circle")
        .attr("class", "point")
    ...
    elementsRef.current.rScale.domain([0, d3.max(monthlyData, (d) => d.value)]).nice();
    ...
    const line = d3.lineRadial()
      .angle((d) => elementsRef.current.angleScale(d.month.abr) + elementsRef.current.angleScale.bandwidth() / 2)
    ...
    elementsRef.current.points
      ...
      .transition(animationRef.current)
      .attr("cx", (d) => {
        const a = elementsRef.current.angleScale(d.month.abr) + elementsRef.current.angleScale.bandwidth() / 2;
    ...
    elementsRef.current.labels.data(monthlyData)
      .attr("text-anchor", (d) => {
        const a = elementsRef.current.angleScale(d.month.abr) + elementsRef.current.angleScale.bandwidth() / 2;
    ...
    d3.select(tooltipRef.current)
      ...
      <div class="bg-white p-3 rounded shadow-lg border border-gray-200 min-w-[200px]">
        ...
        <div class="text-xs mt-2">
          ...
          ${activeYear ? `<div class="text-gray-500">Année: ${activeYear}</div>` : ""}
        </div>
      </div>
      `);
  };
  ...
  return (
    <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <h4 style={{ textAlign: "center", maxWidth: "600px" }}>
        Ce graphique circulaire révèle les tendances mensuelles de popularité des musiques. La courbe et les points interactifs mettent en évidence les pics saisonniers, tandis que le filtre par année permet d'analyser leur évolution dans le temps.
      </h4>
      <div style={{ margin: "1rem", textAlign: "center" }}>
        <label htmlFor="popularity-metric" style={{ marginRight: "0.5rem" }}>
          Choisissez une métrique de popularité :
        </label>
        <select id="popularity-metric" value={selectedMetric} onChange={(e) => setSelectedMetric(e.target.value)}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <div
          ref={tooltipRef}
          style={{ position: "absolute", opacity: 0, pointerEvents: "none", transition: "opacity 0.2s", zIndex: 10 }}
        ></div>
        {years.length > 0 && (
          <div className="w-full max-w-2xl mt-8 px-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium">Filtrer par année:</span>
              <span className={`text-sm font-medium px-3 py-1 rounded ${activeYear ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}`}>
            ...
            <button
              ...
              className={`px-3 py-1 mt-8 rounded text-sm transition-colors ${!activeYear ? "bg-blue-600 text-white shadow-md" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
            >
              Toutes les années
            </button>
          </div>
